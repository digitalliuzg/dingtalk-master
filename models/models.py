...
class dingtalkAccount(models.Model):
    ...
    name = fields.Char()
    access_token = fields.Char(compute='cron_get_access_token')
    corpid = fields.Char(required=True)
    corpsecret = fields.Char(required=True)
    jsapi_ticket = fields.Char("Jsapi Ticket", compute='cron_get_access_token')
    errmsg = fields.Char()
        ...
        for account in accounts:
            ...
            if results['errcode'] == 0 and results['errmsg'] == 'ok':
                ...
                if results2['errcode'] == 0 and results2['errmsg'] == 'ok':
                    ...
                    self.jsapi_ticket = results2['ticket']
    ...
    def btn_enterprise_chat(self):
        for rec in self:
            message = {
                ...
                "msgtype": "text",
                "text": {
                    "content": "我叫佩奇，这是我哥佩不奇5623589611215411141"
                }
            }
...
class dingtalkApp(models.Model):
    ...
    # x = str(int(input('请输入：')*100))
    # y = str(int(x))
    # print('%s元%s角%s分' % (y[:2],y[2],y[3]))
    # print('%s元%s角%s分' % (x[:2], x[2], x[3]))
    # print(y[:2]+ '元'+ 'y[2]'+ '角',+y[3]+ '分')
